...
## implementing deep_dream in keras
...
seeting up the deep_dream configuration
```{r}
...
fetch_loss_and_grabs <- k_function(list(dream), outputs)

eval_loss_and_grads <- function(x) {
  outs <- fetch_loss_and_grabs(list(x))
  loss_value <- outs[[1]]
  grad_values <- outs[[2]]
...
for (shape in successive_shapes) {
  ...
  save.image(img, fname = sprintf("dream_at_scale_%s.png",
...
preprocess_image <- function(imgae_path) {
